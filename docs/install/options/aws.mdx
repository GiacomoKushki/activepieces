...
* MongoDB Atlas instance. Opting out of this will use a local SQLite3 Db.
...
This step will prompt you to create you stack and to populate a series of config options, such as whether or not to provision a MongoDB Atlas instance or use SQLite3.
...
For example, if you'd like to have three availability zones instead of two for the VPC, use an older version of Redis or add some additional security group rules for MongoDB, you can update all of these and more in the `index.ts` file inside the `deploy` folder.
